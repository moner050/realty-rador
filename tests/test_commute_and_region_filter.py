...
def test_commute_map_gangnam():
    """강남역 통근 룩업 맵 30분/60분/90분 반환 테스트."""
    codes_30 = get_sigungu_codes_within_commute(30, "gangnam")
    codes_60 = get_sigungu_codes_within_commute(60, "gangnam")
    codes_90 = get_sigungu_codes_within_commute(90, "gangnam")

    # 30분 이내: 대표 생활권 기준 강남구(11680), 서초구(11650), 송파구(11710)가 포함됨
    assert 11680 in codes_30
    assert 11650 in codes_30
    assert 11710 in codes_30

    # 60분 이내: 마포구(11440), 분당구(41135), 과천시(41290), 구리시(41310), 성동구(11200)가 정상 포함됨
    assert 11440 in codes_60
    assert 41135 in codes_60
    assert 41290 in codes_60
    assert 41310 in codes_60
    assert 11200 in codes_60

    # 도봉구(11320), 일산동구(41285), 남양주시(41360), 파주시(41480)는 60분 이내에서 제외됨
    assert 11320 not in codes_60
    assert 41285 not in codes_60
    assert 41360 not in codes_60

    # 90분 이내: 도봉구(11320), 일산동구(41285), 남양주시(41360), 파주시(41480), 인천 부평구(28237) 포함됨
    assert 11320 in codes_90
    assert 41285 in codes_90
    assert 41360 in codes_90
    assert 41480 in codes_90
    assert 28237 in codes_90

    # 30분 < 60분 < 90분 포함 관계 검증
    assert len(codes_30) < len(codes_60) < len(codes_90)
    assert set(codes_30).issubset(set(codes_60))
