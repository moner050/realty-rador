...
class ApartmentComplex(Base):
    ...
    id = Column(BigInteger().with_variant(Integer, "sqlite"), primary_key=True, autoincrement=True, comment="아파트 단지 일련번호 (PK)")
    complex_code = Column(String(50), unique=True, nullable=True, comment="공공데이터/포털 고유 단지 코드")
    official_name = Column(String(100), nullable=False, comment="아파트 공식 단지명 (예: 여의도 시범아파트)")
    normalized_name = Column(String(100), nullable=False, comment="검색 정규화 단지명 (특수문자/괄호 제거)")

    sido = Column(String(50), nullable=True, comment="시/도 (예: 서울특별시, 경기도)")
    sigungu = Column(String(50), nullable=True, comment="시/군/구 (예: 영등포구, 분당구)")
    dong = Column(String(50), nullable=True, comment="법정동/행정동 (예: 여의도동, 백현동)")
    road_address = Column(String(200), nullable=True, comment="도로명/지번 상세 주소")

    total_households = Column(Integer, nullable=True, comment="총 세대수")
    total_buildings = Column(Integer, nullable=True, comment="총 동수")
    construction_year = Column(Integer, nullable=True, comment="준공 연도 (예: 1971)")
    use_approval_date = Column(String(20), nullable=True, comment="사용승인일자 (YYYY-MM-DD)")
    ...
    created_at = Column(DateTime, default=datetime.now, nullable=False, comment="단지 등록 일시")
    updated_at = Column(DateTime, default=datetime.now, onupdate=datetime.now, nullable=False, comment="단지 수정 일시")
...
class ComplexAlias(Base):
    ...
    id = Column(BigInteger().with_variant(Integer, "sqlite"), primary_key=True, autoincrement=True, comment="단지 별칭 일련번호 (PK)")
    complex_id = Column(BigInteger().with_variant(Integer, "sqlite"), ForeignKey("apartment_complex.id", ondelete="CASCADE"), nullable=False, comment="연결된 아파트 단지 ID (FK)")
    source_id = Column(BigInteger().with_variant(Integer, "sqlite"), ForeignKey("crawl_source.id"), nullable=True, comment="수집 출처 ID (FK)")
    ...
    match_method = Column(String(30), nullable=False, comment="매칭 연산 방식 (ADDRESS_EXACT, NAME_EXACT, FUZZY, MANUAL)")
