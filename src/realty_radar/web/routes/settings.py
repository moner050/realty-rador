...
from realty_radar.web.auth import SESSION_COOKIE_NAME, verify_session_token
...
def get_settings() -> RedirectResponse:
    """이전 설정 URL을 매물 검색 화면으로 보냅니다."""
    return RedirectResponse(url="/", status_code=status.HTTP_303_SEE_OTHER)


@router.post("/inline", status_code=204, response_class=Response, name="update_inline_settings")
def update_inline_settings(
    request: Request,
    ...
    if username:
        ...
        response.set_cookie(
            ...
            value=urllib.parse.quote(json.dumps(profile.to_dict(), ensure_ascii=False)),
            max_age=86400 * 30,
            httponly=False,
            samesite="lax",
        )
